...
export function buildTreeBuffers(treeData, age) {
  ...
  for (const [, nodes] of byBranch) {
    ...
    const frac       = visibleF - visibleInt;   
    ...
    for (let i = 0; i <= Math.min(ringCount, totalSections); i++) {
      ...
      for (let j = 0; j < segments; j++) {
        ...
        const barkNoise =
          Math.sin(i * 1.3 + node.level * 17.7 + ang * 8 + t * 14)
          * 0.04 * (node.level === 0 ? 1.0 : 0.5);
        const finalR = r * (1 + barkNoise * (node.level === 0 ? 0.22 : 0.10));
      ...
      // faces between ring i-1 and ring i
      if (i > 0) {
